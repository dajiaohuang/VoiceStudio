...
- The desktop app no longer gives up on a backend that is still starting: a slow cold start (project on a network drive, a cold `import torch`, a first CUDA load) is kept and narrated instead of being killed at five minutes and reported as "the backend never reported ready" (#1791)
